var Calendar=function(model,options,date){this.Options={Color:'',LinkColor:'',NavShow:!0,NavVertical:!1,NavLocation:'',DateTimeShow:!0,DateTimeFormat:'mmm, yyyy',DatetimeLocation:'',EventClick:'',EventTargetWholeDay:!1,DisabledDays:[],ModelChange:model};for(var key in options){this.Options[key]=typeof options[key]=='string'?options[key].toLowerCase():options[key]}
    ...
    this.Selected.Month=this.Selected.getMonth();this.Selected.Year=this.Selected.getFullYear();this.Selected.Days=new Date(this.Selected.Year,(this.Selected.Month+1),0).getDate();this.Selected.FirstDay=new Date(this.Selected.Year,(this.Selected.Month),1).getDay();if(this.Selected.FirstDay==0){this.Selected.FirstDay=6}else{this.Selected.FirstDay--}
    this.Selected.LastDay=new Date(this.Selected.Year,(this.Selected.Month+1),0).getDay();if(this.Selected.LastDay==0){this.Selected.LastDay=6}else{this.Selected.LastDay--}
    this.Prev=new Date(this.Selected.Year,(this.Selected.Month-1),1);if(this.Selected.Month==0){this.Prev=new Date(this.Selected.Year-1,11,1)}
    ...
    function AddLabels(){var labels=document.createElement('ul');labels.className='cld-labels';var labelsList=["lun","mar","mer","gio","ven","sab","dom"];for(var i=0;i<labelsList.length;i++){var label=document.createElement('li');label.className+="cld-label";label.innerHTML=labelsList[i];labels.appendChild(label)}
    ...
    function AddDays(){function DayNumber(n){var number=document.createElement('p');number.className+="cld-number";number.innerHTML+=n;return number}
        function CheckCurrentDayNumber(n){var dataInformation=new Date();var currentDay=dataInformation.getDate();var number=document.createElement('p');if(n==currentDay){number.className+="cld-number not-current-today"}else{number.className+="cld-number"}
            number.innerHTML+=n;return number}
        var days=document.createElement('ul');days.className+="cld-days";for(var i=0;i<(calendar.Selected.FirstDay);i++){var day=document.createElement('li');day.className+="cld-day prevMonth";var d=i%7;for(var q=0;q<calendar.Options.DisabledDays.length;q++){if(d==calendar.Options.DisabledDays[q]){day.className+=" disableDay"}}
            var dataInformation=new Date();var currentMonth=dataInformation.getMonth();var number;if((calendar.Selected.Month-1)==currentMonth){number=CheckCurrentDayNumber((calendar.Prev.Days-calendar.Selected.FirstDay)+(i+1))}else{number=DayNumber((calendar.Prev.Days-calendar.Selected.FirstDay)+(i+1))}
        ...
        for(var i=0;i<calendar.Selected.Days;i++){var day=document.createElement('li');day.className+="cld-day currMonth";var d=(i+calendar.Selected.FirstDay)%7;for(var q=0;q<calendar.Options.DisabledDays.length;q++){if(d==calendar.Options.DisabledDays[q]){day.className+=" disableDay"}}
            var number=DayNumber(i+1);for(var n=0;n<calendar.Model.length;n++){var evDate=calendar.Model[n].Date;var toDate=new Date(calendar.Selected.Year,calendar.Selected.Month,(i+1));if(evDate.getTime()==toDate.getTime()){number.className+=" eventday";var title=document.createElement('span');title.className+="cld-title";console.log(calendar.Model[n].Title[0]);for(var evSameDay=0;evSameDay<calendar.Model[n].Title.length;evSameDay++){if(typeof calendar.Model[n].Link[evSameDay]=='function'||calendar.Options.EventClick){var a=document.createElement('a');a.setAttribute('href','#');a.innerHTML+=calendar.Model[n].Title[evSameDay];if(calendar.Options.EventClick){var z=calendar.Model[n].Link[evSameDay];if(typeof calendar.Model[n].Link[evSameDay]!='string'){a.addEventListener('click',calendar.Options.EventClick.bind.apply(calendar.Options.EventClick,[null].concat(z)));if(calendar.Options.EventTargetWholeDay){day.className+=" clickable";day.addEventListener('click',calendar.Options.EventClick.bind.apply(calendar.Options.EventClick,[null].concat(z)))}}else{a.addEventListener('click',calendar.Options.EventClick.bind(null,z));if(calendar.Options.EventTargetWholeDay){day.className+=" clickable";day.addEventListener('click',calendar.Options.EventClick.bind(null,z))}}}else{a.addEventListener('click',calendar.Model[n].Link[evSameDay]);if(calendar.Options.EventTargetWholeDay){day.className+=" clickable";day.addEventListener('click',calendar.Model[n].Link[evSameDay])}}
                title.appendChild(a)}else{title.innerHTML+='<a href="'+calendar.Model[n].Link[evSameDay]+'">'+calendar.Model[n].Title[evSameDay]+'</a>'}}
        ...
        for(var i=0;i<(extraDays-calendar.Selected.LastDay);i++){var day=document.createElement('li');day.className+="cld-day nextMonth";var d=(i+calendar.Selected.LastDay+1)%7;for(var q=0;q<calendar.Options.DisabledDays.length;q++){if(d==calendar.Options.DisabledDays[q]){day.className+=" disableDay"}}
            var dataInformation=new Date();var currentMonth=dataInformation.getMonth();var number;if((calendar.Selected.Month+1)==currentMonth){number=CheckCurrentDayNumber(i+1)}else{number=DayNumber(i+1)}
            day.appendChild(number);days.appendChild(day)}
    ...
    AddLabels();AddDays()}
function caleandar(el,data,settings){var obj=new Calendar(data,settings);createCalendar(obj,el);console.log(obj)}